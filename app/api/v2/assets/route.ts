 ...
 */
export const GET = withScope("GET /api/v2/assets", async (req, { scope }) => {
  ...
  try {
    // Canonical source : Supabase, via loadAssetsForScope (V2).
...
export async function POST(req: NextRequest) {
  ...
  const parsedBody = await parseJsonBody(req, createAssetSchema);
  if (!parsedBody.ok) return parsedBody.response;

  const { type, name, run_id, url, threadId, metadata } = parsedBody.data;
