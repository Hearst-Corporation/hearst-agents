...
export async function POST(
  ...
  if (contentLength && contentLength !== "0") {
    const parsed = await parseJsonBody(req, resumeMissionSchema);
  ...
  try {
    ...
    if (runtimeMission) {
      ...
      if (!owns) {
        logger.warn(
          ...
          "Mission resume blocked — ownership mismatch",
        );
      ...
      logger.info(
        ...
        "[MissionsAPI] Runtime mission resumed",
      );
    ...
    if (persistedMission) {
      ...
      logger.info(
        { missionId, userId: redactId(scope.userId) },
        "[MissionsAPI] Persisted mission resumed",
      );
    ...
    if (plannerMission) {
      ...
      logger.info({ missionId }, "[MissionsAPI] Planner mission resumed");
