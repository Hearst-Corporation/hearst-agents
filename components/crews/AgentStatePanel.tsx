import type { AgentRow, RunStats } from "@/lib/types/swarm";
...
export function AgentStatePanel({ agentRows, runStats, lastRunAt, runStatus }: Props) {
  ...
  return (
    <div className="ct-card">
      ...
      >
        {isRunning && <span className="ct-pulse-dot" style={{ display: "inline-block" }} />}
        ...
        {agentRows.length === 0 ? (
          ...
          agentRows.map((agent) => (
            ...
            >
              ...
              <span className={badgeClass(agent.status)}>{agent.statusLabel}</span>
            </div>
          ))
        )}
      </div>
