...
import type { P0Item } from "@/lib/types/swarm";
...
export function DecisionCard({ p0Item, draftText, runId }: Props) {
  ...
  const handleDecision = async (action: "snoozed" | "rejected") => {
    ...
    try {
      ...
      const res = await fetch("/api/crewai/crews/chief-of-staff/decisions", {
        method: "POST",
  ...
  if (!p0Item) {
    return (
      <div className="ct-card" style={{ textAlign: "center", padding: SPACING.xxl }}>
  ...
  return (
    ...
    >
      ...
      {/* From */}
      <div style={{ fontSize: FONT.sm, color: "var(--ct-text-muted)" }}>From: {p0Item.from}</div>
