// SwarmList — depends on @/components/runs/StatusBadge and @/components/ui/Chevron
...
import type { SwarmListItem } from "@/lib/types/swarm";
...
export function SwarmList({ swarms, error }: SwarmListProps) {
  if (error) {
    return (
      <div className="ct-card" style={{ borderColor: "var(--ct-border-accent)" }}>
  ...
  return (
    <div className="ct-card" style={{ padding: 0, overflow: "hidden" }}>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: FONT.base }}>
        ...
        <tbody>
          {swarms.map((s) => (
            <tr key={s.id} style={{ borderBottom: "1px solid var(--ct-border-soft)" }}>
              <td style={tdStyle}>
                <Link
                  href={`/swarms/${s.id}`}
                  className="ct-link"
