...
import type { Tool, ToolBindingInput } from "@/lib/types/swarm";
 ...
 * `agentId` requis pour activer les toggles.
 */
export function ToolPicker({
  ...
  if (availableTools.length === 0) {
    return (
      <p className="ct-placeholder">No tool available. Create some from the Tools page (coming).</p>
    );
  }

  if (!agentId) {
    return <p className="ct-placeholder">Select an agent first to assign tools.</p>;
  }
            ...
            {tools.map((tool) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? "var(--ct-accent-soft)" : "var(--ct-surface-1)",
                    border: isSelected
