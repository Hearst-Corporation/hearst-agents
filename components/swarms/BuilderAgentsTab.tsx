...
import type { AgentInput } from "@/lib/types/swarm";
...
export function BuilderAgentsTab({ agents, onAdd, onUpdate, onRemove }: BuilderAgentsTabProps) {
    ...
    <div className="ct-card">
      ...
      >
        ...
        {!showForm && editingIdx === null ? (
          <button type="button" className="ct-seg-btn primary" onClick={() => setShowForm(true)}>
            + Add
          </button>
        ) : null}
      </div>
      ...
      {agents.length === 0 && !showForm ? (
        <p className="ct-placeholder">No agent yet. Add at least one coordinator to start.</p>
      ) : null}
    </div>
  );
}
