...
import type { AgentInput, TaskInput } from "@/lib/types/swarm";
  ...
  return (
    <div className="ct-card">
        ...
        {!showForm && editingIdx === null ? (
          <>
            ...
            <button
              ...
              title={agents.length === 0 ? "Add an agent first (Agents tab)" : undefined}
              style={{
      ...
      {agents.length === 0 && !showForm ? (
        <p className="ct-placeholder" style={{ color: "var(--ct-text-muted)" }}>
      ...
      {tasks.length === 0 && !showForm && agents.length > 0 ? (
        <p className="ct-placeholder">No task yet. Define what each agent must do.</p>
      ) : null}
    </div>
  );
}
