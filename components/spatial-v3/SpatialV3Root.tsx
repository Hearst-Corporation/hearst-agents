...
function CameraRig() {
  ...
  useFrame((state) => {
    // Si un agent est focus, on se rapproche. Si déployé, on recule. Sinon par défaut.
    const targetZ = activeAgent ? 5.5 : (isExpanded ? 8.5 : 4.95);
    state.camera.position.z = THREE.MathUtils.lerp(state.camera.position.z, targetZ, 0.04);
    
    // Si un agent est focus, on incline légèrement la caméra pour donner de l'ampleur
    const targetRotX = activeAgent ? 0.05 : 0;
  ...
  return (
    <Canvas dpr={[1, 1.8]} gl={{ antialias: true, alpha: false, powerPreference: "high-performance" }}>
      ...
      <Suspense fallback={null}>
        ...
        {/* Plan invisible pour intercepter les clics dans le vide (Retour) */}
        <mesh 
          position={[0, 0, -5]} 
          onClick={(e) => {
  ...
  return (
    <form 
      className={[
        ...
        (isExpanded && !activeAgent) ? "opacity-40 hover:opacity-100 translate-y-0" : "opacity-100 translate-y-0"
      ].join(" ")}
      style={{
        transitionDelay: (isExpanded && !activeAgent) ? "100ms" : "300ms",
      }}
      onSubmit={(e) => {
        e.preventDefault();
        console.log("Orchestration query:", query, "Target:", activeAgent || "Global");
        setQuery("");
      }}
    >
      <div
      ...
      >
        <input
          ...
          placeholder={activeAgent ? `Requête vers ${activeAgent}...` : "Demander une orchestration"}
          aria-label={activeAgent ? `Requête vers ${activeAgent}` : "Demander une orchestration"}
          value={query}
