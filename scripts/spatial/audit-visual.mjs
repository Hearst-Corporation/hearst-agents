...
async function runAudit() {
  ...
  try {
    ...
    if (!response || response.status() !== 200) {
      console.error(
        `${ANSI.red}[ERREUR] La page ${config.url} est introuvable (Status: ${response?.status() || "N/A"}).${ANSI.reset}`,
      );
    ...
    if (updateReference || !fs.existsSync(referencePath)) {
      ...
      console.log(
        `${ANSI.green}[OK] Reference ${updateReference ? "mise a jour" : "creee"}. Re-run pour comparer.${ANSI.reset}`,
      );
      await browser.close();
      process.exit(0);
    }
