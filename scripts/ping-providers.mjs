...
const checks = [
  ...
  // Search / Browse
  {
    ...
    headers: (_k) => ({ "Content-Type": "application/json" }),
    prepBody: (k, b) => JSON.stringify({ ...JSON.parse(b), api_key: k }),
  },
  {
    name: "Exa",
    env: "EXA_API_KEY",
  ...
  },
  {
    name: "PeopleDataLabs",
    env: "PDL_API_KEY",
    url: "https://api.peopledatalabs.com/v5/account/usage",
    headers: (k) => ({ "X-Api-Key": k }),
  },
  ...
  },
  {
    ...
    headers: (k) => ({ "X-Hume-Api-Key": k }),
  },
  {
    name: "Runway",
    env: "RUNWAY_API_KEY",
    url: "https://api.dev.runwayml.com/v1/organization",
    headers: (k) => ({ Authorization: `Bearer ${k}`, "X-Runway-Version": "2024-11-06" }),
  },
  {
    name: "HeyGen",
    env: "HEYGEN_API_KEY",
  ...
  },
  {
    ...
    url: "https://backend.composio.dev/api/v1/auth_apps",
    headers: (k) => ({ "x-api-key": k }),
  },
  {
    name: "Langfuse",
    env: "LANGFUSE_PUBLIC_KEY",
    url: `${process.env.LANGFUSE_HOST ?? "https://cloud.langfuse.com"}/api/public/health`,
    headers: (k) => ({
      Authorization: `Basic ${Buffer.from(`${k}:${process.env.LANGFUSE_SECRET_KEY ?? ""}`).toString("base64")}`,
    }),
  },
  {
    name: "Arcjet",
  ...
  },
  {
    ...
    url: `https://sentry.io/api/0/organizations/${process.env.SENTRY_ORG ?? ""}/`,
    headers: (k) => ({ Authorization: `Bearer ${k}` }),
  },
  {
    name: "Axiom",
...
for (const r of results) {
  if (r.status !== "fulfilled") {
    console.log("?? agent crash:", r.reason);
    continue;
  }
