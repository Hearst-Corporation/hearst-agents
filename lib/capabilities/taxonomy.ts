...
export const DOMAIN_TAXONOMY: Record<Domain, DomainEntry> = {
  ...
  finance: {
    ...
    providers: ["stripe", "quickbooks", "xero", "shopify", "woocommerce", "bigcommerce"],
    tools: [
      "export_excel",
      "analyze_data",
      "generate_report",
  ...
  developer: {
    ...
    tools: ["list_pull_requests", "list_issues", "create_issue", "list_commits", "create_branch"],
  ...
  analysis: {
    ...
    tools: ["execute_code", "analyze_data", "query_events", "list_cohorts", "track_event"],
...
export function keywordMatches(input: string, keyword: string): boolean {
  ...
  if (kw.length <= 3) {
    return new RegExp(`(^|[\\s.,!?;:()"'«»])${escapeRegex(kw)}($|[\\s.,!?;:()"'«»])`, "i").test(
      lower,
    );
  }
...
export function resolveRetrievalMode(message: string): RetrievalMode | null {
  ...
  const productivityKw = [
    ...DOMAIN_TAXONOMY.productivity.keywords.fr,
    ...DOMAIN_TAXONOMY.productivity.keywords.en,
  ];
...
export function resolveDataIntent(message: string): {
  ...
  return {
    needsCalendar:
      [...CAL_KEYWORDS].some((k) => keywordMatches(message, k)) || domain === "productivity",
    needsGmail:
      [...GMAIL_KEYWORDS].some((k) => keywordMatches(message, k)) || domain === "communication",
