...
test.describe("VideoQuickLaunch — Panel @skip-ci", () => {
  ...
  test("panel s'ouvre via ⌘G @skip-ci", async ({ page }) => {
    ...
    await expect(page.getByRole("dialog", { name: /lancement rapide vidéo/i })).toBeVisible();
  });
  ...
  test("ESC ferme le panel @skip-ci", async ({ page }) => {
    ...
    await expect(page.getByRole("dialog", { name: /lancement rapide vidéo/i })).toHaveAttribute(
      "aria-hidden",
      "true",
    );
  });
