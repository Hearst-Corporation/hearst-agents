...
export async function getHubSession(): Promise<HubSession | null> {
  ...
  if (window.parent && window.parent !== window) {
    return new Promise<HubSession | null>((resolve) => {
      const reqId = `${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
      let settled = false;

      const onMessage = (e: MessageEvent): void => {
        const data = e.data as { type?: string; reqId?: string; session?: HubSession | null } | null;
        if (!data || data.type !== POSTMSG_RES || data.reqId !== reqId) return;
        settled = true;
 ...
 */
export function onHubSessionChange(cb: (s: HubSession | null) => void): () => void {
  ...
  const onMessage = (e: MessageEvent): void => {
    const data = e.data as { type?: string; session?: HubSession | null } | null;
    if (!data || data.type !== POSTMSG_CHANGE) return;
...
export async function bridgeHubSession(supabaseClient: any): Promise<boolean> {
