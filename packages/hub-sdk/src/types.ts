...
export interface HearstHub {
  ...
  files: {
    save(a: { suggestedName: string; data: string; mime?: string }): Promise<boolean>;
    open(a?: { accept?: string }): Promise<{ name: string; data: string } | null>;
  };
  secrets: {
  ...
  notify(a: { title: string; body?: string }): Promise<void>;
