...
function buildCapabilities() {
    const cap = [];
    if (typeof window === "undefined" || !window.hearstHub)
        return cap;
    const hub = window.hearstHub;
    if (hub.storage)
        cap.push("storage");
    if (hub.files)
        cap.push("files");
...
function buildSnapshot() {
    ...
    if (window.hearstHub) {
        ...
        const ctx = window.hearstHub.context;
        return {
            isHub: true,
            ready: true,
            accent: ctx?.accent,
...
function contextKey() {
    ...
    const c = window.hearstHub.context;
    if (!c)
        return "hub-no-ctx";
